...
functional_stmt: (Struct_noun func_verb multi_flow)
                 ((from Struct_noun to Struct_noun)|(to Struct_noun)|(from Struct_noun)|(and func_verb it to (flow|Struct_noun)))?;
energizing_stmt: Struct_noun func_verb multi_flow and func_verb (it|them) to multi_flow;
...
func_verb: 'contains'|'imports'|'exports'|'converts'|'transfers'|'receives'|'energizes'|'deenergizes'|'distributes'|'separates'|'couples'|'stores'|'supplies'|'connected';
