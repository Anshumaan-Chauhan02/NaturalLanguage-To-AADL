...
public class Visitor<Object> extends AbstractParseTreeVisitor<Object> implements TextToAADLVisitor<Object> {
    String[] system_names = new String[30];
    Integer[] sys_connection = new Integer[30];
    String[] created_systems = new String[30];
    String[][] system_declaration = new String[30][2];
    String[] system_subcomponents = new String[30];
    String[] portnames = new String[30];
    int index_ports = 0;
    int index_subcomponents = 0;
    String[] connections = new String[30];
    int index_connections = 0;
    String[] sys_features = new String[30];
...
//        }
     ...
     */
    @Override
    public Object visitStructural_stmts(TextToAADLParser.Structural_stmtsContext ctx) {
        return visitChildren(ctx);
    }
...
//            String nodes=visitStruct_multinoun(ctx.struct_multinoun()).toString();
//            StringTokenizer stnodes= new StringTokenizer(nodes,"[],");
//            while(stnodes.hasMoreTokens()) {
//
//                System.out.println(stnodes.nextToken().replaceAll("\\s", ""));
//            }
//        System.out.println(ctx.struct_multinoun().getText());
//        System.out.println(ctx.struct_multinoun().and()==null);
     ...
     */
    @Override
    public Object visitConnection_stmt(TextToAADLParser.Connection_stmtContext ctx) {
        boolean already_features_main_comp=false;
        String conn_comp=ctx.Struct_noun().getText();
        for(int i=0;i<=index_features-1;i++)
        {
            String comp_name=sys_features[i];
            if(comp_name.equals(conn_comp))
            {
                already_features_main_comp=true;
            }
        }
        String newdecl = "";
        boolean done_features=false;
        if(!already_features_main_comp)
        {
            for(int i=0;i<=index-1;i++)
            {
                if(system_names[i].equals(conn_comp))
                {
                    for(int j=0;j<=system_declaration[i][0].length()-1;j++)
                    {
                        ...
                        {
                            newdecl=newdecl+system_declaration[i][0].charAt(j);
                        }
                    }
                    system_declaration[i][0]=newdecl;
        ...
        StringTokenizer ret_comp=new StringTokenizer(ctx.struct_multinoun().getText());
        while(ret_comp.hasMoreTokens())
        {   newdecl="";
            boolean already_features_each_comp=false;
            String connected_to=ret_comp.nextToken();
            if((connected_to.equals("and"))||(connected_to.equals(",")))
            {
            }
            else
            {
                ...
                if(!already_features_each_comp)
                {    done_features=false;
                    for(int i=0;i<=index-1;i++)
                    {
                        if(system_names[i].equals(connected_to))
                        {
                            for(int j=0;j<=system_declaration[i][0].length()-1;j++)
                            {
                                if((system_declaration[i][0].charAt(j)=='\n')&&(!done_features))
                ...
                //struct noun out data port
                String dec_conn_comp="";
                String dec_to_conn="";
                for(int i=0;i<=index-1;i++)
                {
                    if(conn_comp.equals(system_names[i]))
                    {  boolean new_line=false;
                        int number_of_newline=0;
                        ...
                        for(int j=0;j<=system_declaration[i][0].length()-1;j++)
                        {
                            ...
                            if((number_of_newline==2)&&(!new_line))
                            {
                                index_new_line=j;
                                new_line=true;
                            }
                        ...
                        for(int j=0; j<=system_declaration[i][0].length()-1;j++)
                        {
                            ...
                            {
                                dec_conn_comp=dec_conn_comp+system_declaration[i][0].charAt(j);
                            }
                        }
                        system_declaration[i][0]=dec_conn_comp;
                    }
                }
                //multinoun comp in data port
                for(int i=0;i<=index-1;i++)
                {
                    if(connected_to.equals(system_names[i]))
                    {   boolean new_line=false;
                        int number_of_newline=0;
                        int index_new_line=0;
                        for(int j=0;j<=system_declaration[i][0].length()-1;j++)
                        ...
                        for(int j=0; j<=system_declaration[i][0].length()-1;j++)
                        {
                            ...
                            {
                                dec_to_conn=dec_to_conn+system_declaration[i][0].charAt(j);
                            }
                        }
                        system_declaration[i][0]=dec_to_conn;
                    }
                }
...
//                            System.out.println("Comp1= "+stn[0]+" Comp 2= "+stn[1]+ " Overall comp= " +comp);
    ...
    public Object visitFunctional_stmt(TextToAADLParser.Functional_stmtContext ctx) {
        ...
        String funcverbs = ctx.FUNC_VERB(0).getText();
        String func_verb_used = "";
        String fv = funcverbs;
        if (fv.equals("imports") || fv.equals("receives") || fv.equals("transfers") || fv.equals("distributes")) {
            func_verb_used = fv;
        }
        ...
        if (number_of_sn == 1) {
            if (func_verb_used.equals("imports") || fv.equals("receives"))
                    ...
                    if ((var_count_newline == 2) && (newline_var)) {
                        ...
                        for (int j = 0; j <= number_of_in_ports[0] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                while (stp.hasMoreTokens()) {
                                    ...
                                    if (flows[j].replaceAll("\\s","").equals(portname_current.replaceAll("\\s",""))) {
                                        if_named_port = true;
                                    }
                                }
                                if (if_named_port) {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_from_another_system" + ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_from_another_system";
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }

                            } else {
                                portnames[index_ports] = system_names[ind] + " " + flows[j].replaceAll("\\s","");
                                index_ports += 1;
                                newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": in data port;";
                            }
                        }
                        for (int j = 0; j <= number_of_out_ports[0] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_to_another_system" + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_to_another_system";
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }
                    ...
                    if ((var_count_newline == 2) && (newline_var)) {
                        ...
                        for (int j = 0; j <= number_of_in_ports[struct_val] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    ...
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_from_" + temp + ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_from_" + temp;
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }
                        ...
                        for (int j = 0; j <= number_of_out_ports[struct_val] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_to_" + start + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_to_" + start;
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }
                    ...
                    if ((var_count_newline == 2) && (newline_var)) {
                        ...
                        for (int j = 0; j <= number_of_in_ports[struct_val] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_from_" + start + ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_from_" + start;
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","")+ ": in data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }
                        ...
                        for (int j = 0; j <= number_of_out_ports[struct_val] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    ...
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_to_" + temp + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_to_" + temp;
                                } else {
                                    newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + ": out data port;";
                                    portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","");
                                }
                    ...
                    if ((var_count_newline == 2) && (newline_var)) {
                        ...
                        for (int j = 0; j <= number_of_in_ports[struct_val] - 1; j++) {
                            ...
                            if (already_in_ports) {
                                ...
                                if (if_named_port) {
                                    if (flow_going == 0) {
                                        newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_from_" + second + ": in data port;";
                                        portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_from_" + second;
                                    }
                        ...
                        for (int j = 0; j <= number_of_out_ports[struct_val] - 1; j++) {
                            for (int k = 0; k <= index_ports - 1; k++) {
                                StringTokenizer stp = new StringTokenizer(portnames[k]);
                                String port_comp = stp.nextToken();
//                                System.out.println("Portname comp="+port_comp);
//                                System.out.println("Sys name="+system_names[ind]);
                                if (port_comp.equals(system_names[ind])) {
                                    already_in_ports = true;
                                    index_portnames = k;
                                }
                            ...
                            if (already_in_ports) {
                                ...
                                while (stp.hasMoreTokens()) {
                                    String portname_current = stp.nextToken();
//                                    System.out.println("Compared with = "+portname_current);
//                                    System.out.println("Flow is = "+flows[j]);
                                    if (flows[j].replaceAll("\\s","").equals(portname_current.replaceAll("\\s",""))) {
//                                        System.out.println("Matched");
                                ...
                                if (if_named_port) {
                                    if (flow_going == 0) {
                                        newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_to_" + last + ": out data port;";
                                        portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_to_" + last;
                                    }
                                    if (flow_going == 1) {
                                        newdecl = newdecl + "\n\t\t" + flows[j].replaceAll("\\s","") + "_to_" + start + ": out data port;";
                                        portnames[index_portnames] = portnames[index_portnames] + " " + flows[j].replaceAll("\\s","") + "_to_" + start;
                ...
                for (int j = 0; j <= number_of_in_ports - 1; j++) {
                    ...
                    if (already_in_ports) {
                        ...
                        if ((cap_same == j) && (did_cap)) {
                            ...
                            if (if_named_port) {
                                String temp= flows[j].replaceAll("\\s","");
                                newdecl = newdecl + "\n\t\t" +temp+ "_from_another_system" + ": in data port;";
                                portnames[index_portnames] = portnames[index_portnames] + " " +temp+"_from_another_system";
                            } else {
